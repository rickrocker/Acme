var CountryStateDropdowns = {

    Render: function (jquerySelectObjCountry, jquerySelectObjState) {

        //hide state until its populated
        jquerySelectObjState.parent().hide();
        //$("label[for='" + jquerySelectObjState.attr('id') + "']").hide();
        // jquerySelectObjState.hide();

        if (jquerySelectObjCountry) {
            this.populateCountryDropdown(jquerySelectObjCountry);
            if (jquerySelectObjState) {          
                this.addClickEvents(jquerySelectObjCountry, jquerySelectObjState);
            }
        }
    },

    addClickEvents: function (jquerySelectObjCountry, jquerySelectObjState) {
        var parent = this;
        jquerySelectObjCountry.on('change', '', function (e) {
            parent.populateStateDropdown(jquerySelectObjState, jquerySelectObjCountry.find(":selected").val());
        });
    },

    populateCountryDropdown: function (jquerySelectObjCountry) {
        jquerySelectObjCountry.append('<option value=' + 0 + '>- Select a Country -</option>');
        $.each(CountriesList, function (key, value) {
            jquerySelectObjCountry.append('<option value=' + key + '>' + value + '</option>');
        });
    },

    populateStateDropdown: function (jquerySelectObjState, countryCode) {
        jquerySelectObjState.parent().show();
        //$("label[for='" + jquerySelectObjState.attr('id') + "']").show();
        //jquerySelectObjState.show();

        jquerySelectObjState.empty()
        jquerySelectObjState.append('<option value=' + 0 + '>- Select a State -</option>');

        $.each(StatesList[countryCode], function (key, value) {
            jquerySelectObjState.append('<option value=' + value.code + '>' + value.name + '</option>');
        });
    }
